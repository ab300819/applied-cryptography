...
*			Copyright Peter Gutmann and Stuart Woolford 1996-1997			*
...
static void threadtime( void )
{
    ...
    if (!rc) {
        addRandomLong ( procstat );
        addRandomBuffer( ( BYTE * ) procstat, sizeof( qsPtrRec_t ) );
    }
...
void fastPoll( void )
	...
	addRandomBuffer( ( BYTE * ) querybuffer, sizeof( querybuffer ) );
	DosGetDateTime( &dt );
	addRandomBuffer( ( BYTE * ) &dt, sizeof( DATETIME ) );

	/* Process and Thread information */
	DosGetInfoBlocks( &ptib, &ppib );
	addRandomLong( ptib );
	addRandomWord( ppib );
	addRandomBuffer( ( BYTE * ) ptib, sizeof( TIB ) );  /* TIB */
	addRandomBuffer( ( BYTE * ) ppib, sizeof( PIB ) );  /* PIB */
	addRandomBuffer( ( BYTE * ) ptib->tib_ptib2, sizeof( TIB2 ) ); /* TIB2 */

	/* Command line string */
	addRandomBuffer( ppib->pib_pchcmd, strlen( ppib->pib_pchcmd ) );

	/* Environment string */
	addRandomBuffer( ppib->pib_pchenv, strlen( ppib->pib_pchenv ) );
...
void slowPoll( void )
	...
	while( ( hwndNext = WinGetNextWindow( henum ) ) != NULLHANDLE )
		{
		addRandomLong( hwndNext );

		WinQueryWindowProcess( hwndNext, &pib, &tib );
		addRandomBuffer( ( BYTE * ) &tib, sizeof( TIB ) );
		addRandomBuffer( ( BYTE * ) &pib, sizeof( PIB ) );
		WinQueryWindowRect( hwndNext, &rcl );
		addRandomBuffer( ( BYTE * ) &rcl, sizeof( RECTL ) );
		}
